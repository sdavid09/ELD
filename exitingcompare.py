...
my_bucket = s3_res.Bucket('eldimageexiting')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        #    #print (cur_bucket, bucket2, key, x)
            ...
            response = index_faces('eldimageexiting', 'eldimageinside', key, x)
        ...
        #    print response
            try:
                    ...
                    if response[1][0]['Similarity'] > 70 :# print only similarity 
                        flag =1
                        print "MATCH!"
                        s3del = boto3.client('s3')
                        s3del.delete_object(Bucket='eldimageinside', Key=x)
